...
function TurnoJogador(turno) {
    var divJog1 = document.querySelector("#jog1");
    var divJog2 = document.querySelector("#jog2");
    if (turno == 1) {
        divJog2.classList.remove('Turno');
        divJog1.classList.add('Turno');
        }
    else if (turno == 2) {
        divJog1.classList.remove('Turno');
        divJog2.classList.add('Turno');
    }
}
...
function jogarServidor(jogoId, posicao) {
    ...
    xhttp.onreadystatechange = function () {
        ...
        if (this.readyState == 4) {
            ...
            if (this.status == 500) {
                alert("Não foi possivel executar a jogada");
            }
...
function MontarTabuleiro(Tabuleiro) {
    TurnoJogador(Tabuleiro.turno);
    tabuleiroObj = Tabuleiro;
    var turnoAtual = turnoAtual + 1;
    ...
    //COLOCAR AQUI O VENCEDOR!!
    var turnoA = document.getElementById("turno");
    turnoA.innerText = turnoAtual;
    if (Tabuleiro.winner != 0) {
        if (Tabuleiro.winner == 1) {
            alert("Parabéns jogador 1, voce é o vencedor");
        }
        else if (Tabuleiro.winner == 2) {
            alert("Parabéns jogador 2, voce é o vencedor");
        }
        else {
            alert("JOGO EMPATADO!");
        }
        $('#Tabuleiro').addClass("disabled");
        turnoAtual = 0;
    }
    else
        $('#Tabuleiro').removeClass("disabled");
